...
* For all developers: If you have `direnv` installed and configured, you can copy the included `.envrc.example` to `.envrc` and allow it to run. This takes care of the [dev env setup](dev-setup-target) and [pre-commit hook installation](hook-install-target). Without `direnv`, you can also run `make dev` to create/update the development environment. Remember to source it via `source .venv/bin/activate` afterwards.
